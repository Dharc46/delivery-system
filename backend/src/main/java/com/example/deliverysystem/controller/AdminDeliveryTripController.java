...
public class AdminDeliveryTripController {
    ...
    public ResponseEntity<DeliveryTripDTO> getActiveForShipper(@RequestParam Long shipperId) {
        DeliveryTripDTO trip = deliveryTripService.findActiveTripForToday(shipperId)
            .orElseThrow(() -> new ResourceNotFoundException("No active delivery trip found for shipperId=" + shipperId));
